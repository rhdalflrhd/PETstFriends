...
jindo.$Class = function(oDef) {
	...
	typeClass.extend = jindo.$Class.extend;return typeClass
};
...
jindo.$$ = jindo.cssquery = (function() {
	...
	var getUID4HTML = function(oEl) {
		...
		validUID[nUID] = oEl;return nUID
	};
	var getUID4XML = function(oEl) {
		var oAttr = oEl.getAttribute("_cssquery_UID");
		var nUID = safeHTML ? (oAttr && oAttr[0]) : oAttr;
		if (!nUID) {
			nUID = UID++;oEl.setAttribute("_cssquery_UID", safeHTML ? [ nUID ] : nUID)
		}
		return nUID
	};
	...
	var restoreKeys = function(sQuery, bOnlyAttrBrace) {
		...
		do {
			...
			sQuery = sQuery.replace(rRegex, function(sKey) {
				if (oKeys[sKey]) {
					bChanged = true;return oKeys[sKey]
				}
				return sKey
			})
		} while (bChanged);
	...
	var getExpression = function(sBody) {
		...
		var sBody = sBody.replace(/:([\w-]+)(\(([^)]*)\))?/g, function(_1, sType, _2, sOption) {
			...
			case "last-of-type":
				sType = (sType == "first-of-type" ? "nth-of-type" : "nth-last-of-type");sOption = 1;case "nth-of-type":
		...
		oRet.returnsTAG = sTagName && sTagName != "*" ? "oEl.tagName == " + oRet.quotTAG + " && " : "";return oRet
	};
	var splitToParts = function(sQuery) {
		...
		var sBody = sQuery.replace(/(.*?)\s*(!?[+>~ ]|!)\s*/g, function(_, sBody, sRelative) {
			...
			sRel = sRelative.replace(/\s+$/g, "") || " ";return ""
		});
		if (sBody) {
			aParts.push({
				rel : sRel,
	...
	var compileParts = function(aParts) {
		...
		eval("var fpCompiled = " + sFunc + ";");return fpCompiled
	};
	...
	var old_cssquery = function(sQuery, oParent, oOptions) {
		...
		for (var r = 0, rp = debugOption.repeat; r < rp; r++) {
			aRet = (function(sQuery, oParent, oOptions) {
				...
				getUID = bXMLDocument ? getUID4XML : getUID4HTML;clearKeys();
				var aSplitQuery = backupKeys(sQuery).split(/\s*,\s*/);
				var aResult = [];
				var nLen = aSplitQuery.length;
				for (var i = 0; i < nLen; i++) {
				...
				unsetNodeIndexes();return aResult
	...
	cssquery.useCache = function(bFlag) {
		if (typeof bFlag != "undefined") {
			bUseResultCache = bFlag;cssquery.clearCache()
		}
	...
	cssquery.xpath = function(sXPath, oParent) {
		var sXPath = sXPath.replace(/\/(\w+)(\[([0-9]+)\])?/g, function(_1, sTag, _2, sTh) {
			sTh = sTh || "1";return ">" + sTag + ":nth-of-type(" + sTh + ")"
		});
...
jindo.$Agent.prototype.navigator = function() {
	...
	b.mie = g("PPC", n) || g("Smartphone", n) || g("IEMobile", n);try {
		if (b.ie) {
			if (d > 0) {
				h = d;
...
jindo.$A.prototype.unshift = function(a) {
	this._array.unshift.apply(this._array, Array.prototype.slice.apply(arguments));return this._array.length
};
...
jindo.$A.prototype.reverse = function() {
	this._array.reverse();return this
};
...
jindo.$Ajax = function(c, f) {
	...
	case "get":
	case "post":
		d.method = d.type;d.type = "xhr";case "xhr":
		this._request = b();
		break;case "flash":
		...
		d.method = "get";this._request = new jindo.$Ajax.JSONPRequest(jindo.$Fn(this.option, this).bind());
		break;case "iframe":
...
jindo.$Ajax.prototype.request = function(c) {
			...
			if (window.navigator.userAgent.match(/(?:MSIE) ([0-9.]+)/)[1] == 6 && d.async) {
				var o = function(a) {
					if (h.readyState == 4 && !q._is_abort) {
						...
						q._onload(a);clearInterval(q._interval);
	...
	this._test_url = b;h.send(g);return this
};
...
jindo.$Ajax.prototype.abort = function() {
	try {
		...
		this._is_abort = true;this._request.abort()
	} finally {
		this._status--
	}
	return this
...
jindo.$Ajax.RequestBase = jindo.$Class({
	...
	_getCallbackInfo : function() {
		...
		if (this.option("callbackid") != "") {
			var a = 0;
			do {
				b = "_" + this.option("callbackid") + "_" + a;a++
...
jindo.$Ajax.JSONPRequest = jindo.$Class({
	...
	_callback : function(b) {
		...
		var a = this;
		this.responseJSON = b;this.onload(this);setTimeout(function() {
			a.abort()
		}, 10)
	},
	abort : function() {
...
jindo.$Ajax.FrameRequest = jindo.$Class({
	...
	_callback : function(d, b, c) {
		...
		this.status = 200;
		this.responseText = b;
		this._respHeaderString = c;c.replace(/^([\w\-]+)\s*:\s*(.+)$/m, function(f, e, g) {
			a._respHeaders[e] = g
	...
	send : function(d) {
		...
		e = "{" + e.join(",") + "}";b.push("&id=" + c.id);b.push("&header=" + encodeURIComponent(e));b.push("&proxy=" + encodeURIComponent(this.option("proxy")));b.push("&domain=" + this._domain);b.push("&url=" + encodeURIComponent(this._url.replace(l, "")));b.push("#" + encodeURIComponent(d));
...
jindo.$Json.fromXML = function(a) {
	...
	a = a.replace(/<(\?|\!-)[^>]*>/g, "");a.replace(m, f);return jindo.$Json(d)
};
...
jindo.$Json.prototype.toXML = function() {
	var a = function(l, e) {
		...
		case "undefined":
		case "null":
			return h("");case "number":
			return h(l);case "string":
			if (l.indexOf("<") < 0) {
...
jindo.$Cookie.prototype.set = function(e, f, d, a, b) {
	...
	document.cookie = e + "=" + escape(f) + c + "; path=" + b + (a ? "; domain=" + a : "");return this
};
...
jindo.$Element.prototype.visible = function(a, b) {
	if (typeof a != "undefined") {
		this[a ? "show" : "hide"](b);return this
	}
...
jindo.$Element.prototype.hide = function() {
	this._element.style.display = "none";return this
};
jindo.$Element.prototype.toggle = function(a) {
	this[this.visible() ? "hide" : "show"](a);return this
};
	...
	if (typeof f == "object") {
		...
		for (var d in f) {
			if (f.hasOwnProperty(d)) {
				...
				if (d == "opacity") {
					p == "undefined" ? this.opacity() : this.opacity(s);continue
				}
				if (d == "cssFloat" && IS_IE) {
					d = "styleFloat"
				}
				if ((IS_FF || IS_OP) && (d == "backgroundPositionX" || d == "backgroundPositionY")) {
					...
					s = d == "backgroundPositionX" ? s + " " + b[1] : b[0] + " " + s;this._setCss(n, "backgroundPosition", s)
				} else {
					this._setCss(n, d, s)
				}
			}
...
jindo.$Element.prototype._getCss = function(c, b) {
	...
	jindo.$Element.prototype._getCss = a;return a(c, b)
};
	...
	if (this._element.classList) {
		jindo.$Element.prototype.toggleClass = function(d, c) {
			if (typeof c == "undefined") {
				...
				if (this.hasClass(d)) {
					this.removeClass(d);this.addClass(c)
				} else {
					this.addClass(d);this.removeClass(c)
				}
			}
			return this
		}
	...
	if (a) {
		jindo.$Element.prototype.html = function(h) {
			if (typeof h != "undefined" && arguments.length) {
				...
				case "select":
				case "table":
					e = o.createElement("div");e.innerHTML = "<" + d + ' class="' + g + '">' + h + "</" + d + ">";
					break;case "tr":
				case "thead":
				case "tbody":
				case "colgroup":
					e = o.createElement("div");e.innerHTML = "<table><" + d + ' class="' + g + '">' + h + "</" + d + "></table>";
					break;default:
					l.innerHTML = h;
					break
				}
					...
					if (n) {
						...
						for (var p = n.firstChild; p; p = n.firstChild) {
							if (d == "select") {
								...
								l.appendChild(f);p.removeNode(true)
							} else {
								l.appendChild(p)
							}
						}
			...
			jindo.$Element.prototype.html = function(n) {
				if (typeof n != "undefined" && arguments.length) {
					...
					if (!h.parentNode) {
						...
						case "select":
						case "table":
							l = d.createElement("div");l.innerHTML = "<" + f + ' class="' + g + '">' + n + "</" + f + ">";
							break;case "tr":
						case "thead":
						case "tbody":
						case "colgroup":
							l = d.createElement("div");l.innerHTML = "<table><" + f + ' class="' + g + '">' + n + "</" + f + "></table>";
							break;default:
							h.innerHTML = n;
							break
						}
...
jindo.$Element.prototype.outerHTML = function() {
	...
	c.insertBefore(f, d);
	f.style.display = "none";f.appendChild(d);
	var b = f.innerHTML;
	c.insertBefore(d, f);c.removeChild(f);return b
};
...
jindo.$Element.prototype.appear = function(b, c) {
	...
	if (a.hasTransition) {
		jindo.$Element.prototype.appear = function(g, h) {
			...
			h = h || function() {};
			var f = function() {
				h();this.show();this.removeEventListener(a.name + "End", arguments.callee, false)
			};
			var e = this._element;
			var d = this;
			if (!this.visible()) {
				e.style.opacity = e.style.opacity || 0;d.show()
			}
			e.addEventListener(a.name + "End", f, false);
			e.style[a.name + "Property"] = "opacity";
			e.style[a.name + "Duration"] = g + "s";
			e.style[a.name + "TimingFunction"] = "linear";setTimeout(function() {
				e.style.opacity = "1"
			}, 1);return this
		}
	} else {
...
jindo.$Element.prototype.disappear = function(b, c) {
	...
	if (a.hasTransition) {
		jindo.$Element.prototype.disappear = function(g, h) {
			...
			h = h || function() {};
			var f = function() {
				h();this.removeEventListener(a.name + "End", arguments.callee, false);d.hide()
			};
			var e = this._element;
			e.addEventListener(a.name + "End", f, false);
			e.style[a.name + "Property"] = "opacity";
...
jindo.$Element.prototype.offset = function(b, a) {
	...
	if (typeof b == "number" && typeof a == "number") {
		...
		e.style.left = parseInt(this.css("left"), 10) + l.left + "px";return this
	}
...
jindo.$Element._append = function(b, a) {
	...
	b._element.appendChild(a);return b
};
...
jindo.$Element.prototype.replace = function(b) {
	...
	if (a && a.replaceChild) {
		a.replaceChild(d.$value(), c);return d
	}
	var d = d.$value();
	a.insertBefore(d, c);a.removeChild(c);return d
};
jindo.$Element.prototype.appendTo = function(a) {
	var b = jindo.$Element(a);
	jindo.$Element._append(b, this._element);return b
};
jindo.$Element.prototype.prependTo = function(a) {
	jindo.$Element._prepend(a, this);return jindo.$Element(a)
};
jindo.$Element.prototype.before = function(a) {
	var b = jindo.$Element(a);
	var c = b.$value();
	this._element.parentNode.insertBefore(c, this._element);return b
};
jindo.$Element.prototype.after = function(a) {
	var b = this.before(a);
	b.before(this);return b
};
...
jindo.$Element.prototype.fireEvent = function(c, b) {
	function d(h, e) {
		h = (h + "").toLowerCase();var f = document.createEventObject();
		if (e) {
			for (k in e) {
		...
		this._element.fireEvent("on" + h, f);return this
	}
	function a(l, g) {
		var m = "HTMLEvents";
		l = (l + "").toLowerCase();
		...
		if (g) {
			...
			g.meta = g.meta || false;switch (m) {
			case "MouseEvent":
				f = document.createEvent(m);f.initMouseEvent(l, true, true, null, g.detail || 0, g.screenX || 0, g.screenY || 0, g.clientX || 0, g.clientY || 0, g.ctrl, g.alt, g.shift, g.meta, g.button, g.relatedElement || null);
				break;case "KeyboardEvent":
				if (window.KeyEvent) {
					f = document.createEvent("KeyEvents");f.initKeyEvent(l, true, true, window, g.ctrl, g.alt, g.shift, g.meta, g.keyCode, g.keyCode)
				} else {
					try {
				...
				break;default:
				f = document.createEvent(m);f.initEvent(l, true, true)
			}
		} else {
			f = document.createEvent(m);f.initEvent(l, true, true)
		}
		this._element.dispatchEvent(f);return this
	}
	jindo.$Element.prototype.fireEvent = (typeof this._element.dispatchEvent != "undefined") ? a : d;return this.fireEvent(c, b)
};
jindo.$Element.prototype.empty = function() {
	jindo.$$.release();this.html("");return this
};
jindo.$Element.prototype.remove = function(a) {
	jindo.$$.release();jindo.$Element(a).leave();return this
};
jindo.$Element.prototype.leave = function() {
	var a = this._element;
	if (a.parentNode) {
		jindo.$$.release();a.parentNode.removeChild(a)
	}
	jindo.$Fn.freeElement(this._element);return this
};
...
jindo.$Element.prototype.undelegate = function(c, a, b) {
	this._unbind(c, a, b);return this
};
		...
		if (b && f) {
			...
			if (b) {
				...
				for (var h = 0, g = n.length; h < g; h++) {
					d = n[h];jindo.$Element._unEventBind(a, d, a["_delegate_" + d + "_func"]);jindo.$Element._delDelegateInfo(a, "_delegate_" + d);jindo.$Element._delDelegateInfo(a, "_delegate_" + d + "_func")
				}
...
jindo.$Element._deleteEvent = function(a, d, c) {
	...
	jindo.$Element._unEventBind(a, d, a["_delegate_" + d + "_func"]);jindo.$Element._delDelegateInfo(a, "_delegate_" + d);jindo.$Element._delDelegateInfo(a, "_delegate_" + d + "_func");
	...
	if (typeof e.addEventListener != "undefined" && !jindo.$Fn._resizeEventBugInIE(d)) {
		...
		if (typeof e.attachEvent != "undefined") {
			if (n == "domready") {
				...
				jindo.$Fn._domready(e, o);return this
			} else {
...
jindo.$Fn.prototype.detach = function(f, c) {
	...
	for (var h = 0; h < m.length; h++) {
		...
		o = m[h].func;
		this._events = jindo.$A(this._events).refuse(m[h]).$value();break
	}
	if (typeof d.removeEventListener != "undefined" && !jindo.$Fn._resizeEventBugInIE(c)) {
		...
		if (typeof d.detachEvent != "undefined") {
			if (n == "domready") {
				jindo.$Fn._domready.list = jindo.$Fn._domready.list.refuse(o);return this
			} else {
...
jindo.$Fn.prototype.delay = function(a, b) {
	...
	this._delayKey = setTimeout(this.bind.apply(this, b), a * 1000);return this
};
jindo.$Fn.prototype.setInterval = function(a, b) {
	if (typeof b == "undefined") {
		b = []
	}
	this._repeatKey = setInterval(this.bind.apply(this, b), a * 1000);return this._repeatKey
};
...
jindo.$S.prototype.bytes = function(d) {
	...
	if (h.toLowerCase() == "utf-8") {
		for (c = 0; c < a; c++) {
			...
			if (f && b > g) {
				this._str = this._str.substr(0, c);break
			}
		}
	} else {
		for (c = 0; c < a; c++) {
...
jindo.$S.prototype.parseString = function() {
	...
	for (var b = 0; b < h.length; b++) {
		c = h[b].substring(0, l = h[b].indexOf("=")), d = false;try {
...
jindo.$Template.prototype.process = function(data) {
		...
		if (i % 2) {
			tpl[i] = tpl[i].replace(jindo.$Template.pattern, function() {
				...
				if (m[5]) {
					delete_info = m[4];
					var _aStr = [];
					_aStr.push("var t#=d." + m[5] + "||{},p#=isArray(t#),i#=0;");_aStr.push("for(var x# in t#){");_aStr.push("if(!t#.hasOwnProperty(x#)){continue;}");_aStr.push("	if( (p# && isNaN(i#=parseInt(x#,10))) || (!p# && !t#.propertyIsEnumerable(x#)) ) continue;");_aStr.push("	d." + m[4] + "=t#[x#];");_aStr.push(m[3] ? "d." + m[3] + "=p#?i#:x#;" : "");return _aStr.join("").replace(reg[4], lev++)
				}
				if (m[6]) {
					return "}else{"
				}
	...
	var _aStr = [];
	_aStr.push("var s=[],i=0;");_aStr.push('function isArray(o){ return Object.prototype.toString.call(o) == "[object Array]" };');_aStr.push(tpl);_aStr.push('return s.join("");');
	tpl = eval("false||function(d){" + _aStr.join("") + "}");
	tpl = tpl(data);return tpl
};
	...
	if ((f = typeof h) == "string") {
		...
		if (f == "number") {
			...
			if (f == "object" && h.constructor == Date) {
				(this._date = new Date).setTime(h.getTime());this._date.setMilliseconds(h.getMilliseconds())
			} else {
				this._date = new Date
			}
		}
...
jindo.$Date.prototype.format = function(c) {
	...
	return (c || "").replace(/[a-z]/ig, function g(d) {
		...
		case "d":
		case "j":
			f.j = e.getDate();f.d = (f.j > 9 ? "" : "0") + f.j;return f[d];case "l":
		case "D":
		case "w":
		case "N":
			f.w = e.getDay();f.N = f.w ? f.w : 7;f.D = b.s_day[f.w];f.l = b.day[f.w];return f[d];case "S":
			return (!!(f.S = [ "st", "nd", "rd" ][e.getDate()])) ? f.S : (f.S = "th");case "z":
			f.z = Math.floor((e.getTime() - (new Date(e.getFullYear(), 0, 1)).getTime()) / (3600 * 24 * 1000));return f.z;case "m":
		case "n":
			f.n = e.getMonth() + 1;f.m = (f.n > 9 ? "" : "0") + f.n;return f[d];case "L":
			f.L = a.isLeapYear();return f.L;case "o":
		case "Y":
		case "y":
			f.o = f.Y = e.getFullYear();f.y = (f.o + "").substr(2);return f[d];case "a":
		case "A":
		case "g":
		case "G":
		case "h":
		case "H":
			f.G = e.getHours();f.g = (f.g = f.G % 12) ? f.g : 12;f.A = f.G < 12 ? b.ampm[0] : b.ampm[1];f.a = f.A.toLowerCase();f.H = (f.G > 9 ? "" : "0") + f.G;f.h = (f.g > 9 ? "" : "0") + f.g;return f[d];case "i":
			f.i = (((f.i = e.getMinutes()) > 9) ? "" : "0") + f.i;return f.i;case "s":
			f.s = (((f.s = e.getSeconds()) > 9) ? "" : "0") + f.s;return f.s;case "u":
			f.u = e.getMilliseconds();return f.u;case "U":
			f.U = a.time();return f.U;default:
			return d
		}
	})
};
jindo.$Date.prototype.time = function(a) {
	if (typeof a == "number") {
		this._date.setTime(a);return this
	}
	return this._date.getTime()
};
jindo.$Date.prototype.year = function(a) {
	if (typeof a == "number") {
		this._date.setFullYear(a);return this
	}
	return this._date.getFullYear()
};
jindo.$Date.prototype.month = function(a) {
	if (typeof a == "number") {
		this._date.setMonth(a);return this
	}
	return this._date.getMonth()
};
jindo.$Date.prototype.date = function(a) {
	if (typeof a == "number") {
		this._date.setDate(a);return this
	}
...
jindo.$Date.prototype.hours = function(a) {
	if (typeof a == "number") {
		this._date.setHours(a);return this
	}
	return this._date.getHours()
};
jindo.$Date.prototype.minutes = function(a) {
	if (typeof a == "number") {
		this._date.setMinutes(a);return this
	}
	return this._date.getMinutes()
};
jindo.$Date.prototype.seconds = function(a) {
	if (typeof a == "number") {
		this._date.setSeconds(a);return this
	}
...
jindo.$Window.prototype.resizeTo = function(b, a) {
	this._win.resizeTo(b, a);return this
};
jindo.$Window.prototype.resizeBy = function(b, a) {
	this._win.resizeBy(b, a);return this
};
jindo.$Window.prototype.moveTo = function(b, a) {
	this._win.moveTo(b, a);return this
};
jindo.$Window.prototype.moveBy = function(b, a) {
	this._win.moveBy(b, a);return this
};
